...
$globalSourcePage = $page->global_source()->toPage();
if (!$globalSourcePage) {
    $globalSourcePage = page("global/webflow");
}
$baseBlocks = $globalSourcePage ? $serializeBlocks($globalSourcePage->blocks()) : [];
$localBlocks = $serializeBlocks($page->local_blocks());
$serializedBlocks = array_merge($baseBlocks, $localBlocks);

$props = [
    "pageTitle" => $page->doc_title()->or($globalSourcePage?->doc_title())->or($page->title())->value(),
    "pageDescription" => (string) $page->doc_description()->or($globalSourcePage?->doc_description())->kt(),
    "blocks" => $serializedBlocks,
];
$id = "doc-client-" . $page->id();
?>
